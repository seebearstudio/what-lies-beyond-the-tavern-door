...
const RUNNING_SPEED_MODIFIER : float = 2.5
const SNEAKING_SPEED_MODIFIER : float = 0.5
const SNEAK_RUN_MODIFIER : float = 1.5
const BACKWARD_MOVEMENT_MODIFIER : float = 0.75
...
static var sneaking : bool = false
...
static func handle_slopes(_creature : Creature) -> void:
	if _creature.slope_ray.is_colliding():
		...
		if abs(tangent) <= SLOPE_MAX_ANGLE / 100:
			if not on_slope:
				on_slope = true
		else:
			if on_slope:
...
static func handle_running() -> void:
	if not running:
...
static func handle_sneaking() -> void:
	if not sneaking and not running:
...
static func handle_movement(_creature : Creature) -> void:
	...
	if direction:
		if input_dir == -1.0:
			if sneaking and running:
				set_locomotive_state(_creature,Creature.LOCOMOTIVE_STATE.SNEAK_RUN)
			elif sneaking:
				set_locomotive_state(_creature,Creature.LOCOMOTIVE_STATE.SNEAKING)
			elif running:
...
static func running_modifier(_creature : Creature) -> float:
	if _creature.locomotive_state == Creature.LOCOMOTIVE_STATE.RUNNING:
		return RUNNING_SPEED_MODIFIER
	else:
		return 1.0

static func sneaking_modifier(_creature : Creature) -> float:
	if _creature.locomotive_state == Creature.LOCOMOTIVE_STATE.SNEAKING:
		return SNEAKING_SPEED_MODIFIER
	else:
		return 1.0
